---
description: Flujo completo de release a Google Play (sube versión, commit propio, push develop, fastlane deploy)
argument-hint: <flavor> [flavor...]   (masmedia | fibrazo | eligetv | tvhotel | fanplaytv)
---
...
3. **Push** a `develop`.
4. Para cada flavor indicado: `fastlane deploy target:<flavor>` desde la raíz. Sube a producción con el rollout por defecto del lane: **masmedia al 20%**, **el resto al 100%**. No pases `rollout:` salvo que el usuario lo pida.
5. Al terminar, recuerda al usuario que la release queda **pendiente de publicación manual** en Play Console (publicación gestionada). Ahí se confirma y, en el caso de masmedia, se avanza el rollout (20% → 50% → 100%).

Notas importantes:
- Las notas de "Novedades" se toman de `fastlane/metadata/<flavor>/...` automáticamente. **No las cambies**: masmedia/fibrazo/tvhotel/fanplaytv usan "Correcciones y mejoras" y **eligetv usa un texto de marketing propio**.
